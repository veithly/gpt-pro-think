...
- **Interruption auto-recovery**: during `wait`, error banners and interrupted replies are detected and the CLI auto-appends a continuation turn (default "继续", up to 2×; tune with `--auto-continue-max`, disable with `--no-auto-continue`, change text with `--continue-text`). Recovery appends into the SAME conversation; the final output includes everything. When attempts are exhausted the run exits 4 with `generation_interrupted` — recover by `--continue`-appending manually.
- **Follow up on any conversation**: pass `--continue --conversation-url https://chatgpt.com/c/<id> "question"` to enter THAT conversation and send the prompt as a follow-up (context preserved; the existing tab is steered to the URL automatically).
- **Select a connector / MCP / plugin**: pass `--connector <name>` (e.g. `--connector MacMCPX`) to pick it from the composer tools menu before sending; `--connector none` clears inline pills. Verify with the `ensure-connector` staged sub-command. Selection failures list the menu rows that were visible.
- **Create a conversation branch**: `node ... search.js branch --conversation-url <src-url> ["follow-up question"]` copies the recent history (default 20 messages / 24k chars; `--branch-max-messages` / `--branch-max-chars`) into a NEW chat and optionally asks the question there. The source conversation stays untouched; the new URL is captured automatically, so later `--continue` turns land in the branch. ChatGPT's native 打开新分支 button is not reliably automatable in current builds — this context-carrying fork is the supported mechanism.
